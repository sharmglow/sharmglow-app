...
class MarketplaceCatalogTab(QWidget):
    ...
    def _preview_product_key(self, product):
        return "|".join([
            str(product.external_id or "").strip(),
    ...
    def _refresh_preview(self, products):
        ...
        for i, product in enumerate(self.current_preview_products):
            ...
            for j, val in enumerate(vals):
                item = QTableWidgetItem(str(val or ""))
                if j == 0:
                    item.setData(Qt.ItemDataRole.UserRole, self._preview_product_key(product))
    ...
    def load_catalog(self):
        ...
        try:
            ...
            return
        self._refresh_preview(products)
    ...
    def _selected_preview_product(self):
        ...
        if not items:
            return None
        row_idx = items[0].row()
        item0 = self.preview_table.item(row_idx, 0)
        key = item0.data(Qt.ItemDataRole.UserRole) if item0 else None
        if key:
            for product in self.current_preview_products:
                if self._preview_product_key(product) == key:
                    return product
        if 0 <= row_idx < len(self.current_preview_products):
    ...
    def import_selected_product(self):
        ...
        dlg = ImportMarketplaceProductDialog(product, market, parent=self)
...
class ImportMarketplaceProductDialog(QDialog):
    def __init__(self, product, market_code, parent=None):
        super().__init__(parent)
