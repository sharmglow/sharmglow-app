...
Файл базы данных: sharmglow.db (рядом с .exe или скриптом)
"""
import sqlite3, os, json
from pathlib import Path
from datetime import date

DB_PATH = Path(os.path.dirname(os.path.abspath(__file__))) / "sharmglow.db"
...
class DB:
    ...
    def upsert_marketplace_product_link(
        ...
        row = self.conn.execute(
            ...
            (product_id, marketplace_code, external_offer_id or "", external_sku or "", external_product_id or ""),
        ).fetchone()

        if row:
            self.conn.execute(
                "UPDATE marketplace_product_links SET account_id=?, vendor_code=?, external_name=?, raw_payload_json=?, synced_at=CURRENT_TIMESTAMP WHERE id=?",
                (account_id, vendor_code, external_name, raw_payload_json or "{}", row["id"]),
            )
            self.conn.commit()
            return row["id"]
        ...
        cur.execute(
            ...
            (
                ...
                marketplace_code,
                account_id,
                external_product_id or "",
                external_sku or "",
                external_offer_id or "",
                vendor_code or "",
                external_name or "",
                raw_payload_json or "{}",
            ),
